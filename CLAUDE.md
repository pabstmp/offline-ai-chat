...
- **4 tarefas no registry**: `web_search_digest` (busca multi-query → LLM → markdown datado, com dedup e diff "o que é novo"), `agent_pipeline` (cascata LINEAR de agentes — ver abaixo), `log_rotation` (rotaciona arquivos configuráveis dentro de `FS_WRITE_ROOTS`), `workspace_backup` (copia cron-state + fontes, gzip). Novo tipo = nova entrada no `TASK_REGISTRY`.
- **`agent_pipeline` (cascata de agentes)**: cada `steps[]` é um agente LLM rodado em ordem. Cada passo pode ter **conexão/modelo próprios** e, opcionalmente, fazer **busca web** e/ou **ler um arquivo local** antes do LLM. A saída final = output do último passo; markdown gravado via `safeWriter`, visível na aba como o boletim. Orçamento de timeout: o cap único do `AbortController` cobre o run inteiro; dentro do run divide-se `ctx.timeoutMs / nSteps` por chamada.
  - **Encadeamento — dois modos** (`options.mode`, só dirige a UI): **Standard** (padrão) = prompts em linguagem natural, **sem token**; o passo recebe a saída dos anteriores via **encadeamento automático**. **Avançado** = tokens `{{steps.<id>.output}}` / `{{vars.<k>}}` / `{{date}}` inseridos por botão, `autoChain` configurável, agentes reutilizáveis.
  - **Auto-chain** (`options.autoChain`, default `true`): no `run()`, se o prompt do passo **não** contém `{{steps.` e `idx>0`, prepende `buildPriorContext(...)` (saída dos passos anteriores). Token explícito **suprime** o prepend (evita duplicar) → retrocompatível. A engine decide por `autoChain`+token, não por `mode`.
  - Interpolação (`interpolateTemplate`) é **uma passada, não-recursiva**: output de LLM não injeta tokens; token ausente vira string vazia (não lança).
- **Agentes reutilizáveis ("skills") — `S.agents`**: biblioteca server-side (irmã de `connections`), CRUD `upsertAgent/deleteAgent/resolveAgent` + endpoints `/api/cron/agent-*`. Um agente = persona (`systemPrompt`) + `defaultPrompt` (sobrescrevível) + `connectionId` + `model` + `tools` + `temperature`. Um passo com `agentId` herda essa config (overrides do passo vencem); sem `agentId` é inline. Não guarda segredo (referencia uma conexão). `ctx.resolveAgent` no `run()`; `getPublicState().agents` pra UI.
- **Leitura segura no cron (`safeReader`)**: a leitura de arquivos dos passos usa `safeReader.readSafeTextFile` (injetado em `taskDeps`), confinada a **WORKSPACE_ROOTS** (whitelist de LEITURA, separada de `FS_WRITE_ROOTS`) — reaproveita `resolveSafePath` + limite `MAX_FILE_BYTES` + guard de binário. `getPublicState` expõe `workspaceRoots` pra UI montar o seletor de pasta de leitura.
