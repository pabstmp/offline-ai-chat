...
**Única dep externa**: `pdfjs-dist` (extração de PDF server-side).
...
Pra deploy single-user local, não exige whitelist. Se `WORKSPACE_ROOTS` env vazio, aceita qualquer pasta. Path traversal guard sempre ativo.
...
for f in app.js server.js modules/*.js modules/ui/*.js modules/workspace/*.js modules/rag/*.js; do
  node --check "$f" 2>/dev/null && echo "$f OK" || echo "FAIL $f"
done
...
Não há suite formal. Tests são scripts ad-hoc em `/tmp/*.js` usando Playwright (instalado em `/tmp/node_modules`).
